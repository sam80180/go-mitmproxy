...
type netflow7PacketBuilder struct {
	baseNetflowPacketBuilder
	router_sc net.IP
} // end type

func NewNetflow7PacketBuilder(cache *cachier.Cache[any], addr string) *netflow7PacketBuilder {
	baseBuilder := initBaseNetflowPacketBuilder(cache, nil)
	addon := &netflow7PacketBuilder{baseNetflowPacketBuilder: baseBuilder}
	...
	return addon
} // end NewNetflow7PacketBuilder()

func (builder *netflow7PacketBuilder) buildV7Record(srcIp net.IP, SrcPort int, dstIp net.IP, DstPort int, dur time.Duration, payload []byte) []byte {
	b := make([]byte, 52)
	...
	b[44] = 32                                                                      // src_mask
	b[45] = 32                                                                      // dst_mask
	copy(b[48:52], builder.router_sc)                                               // router_sc
	return b
} // end buildV7Record()

func (builder *netflow7PacketBuilder) buildV7Header(flowCount uint16) ([]byte, error) {
	b := make([]byte, 24)
	binary.BigEndian.PutUint16(b[0:2], nf7.Version)
...
func (builder *netflow7PacketBuilder) Build(srcAddr, dstAddr net.Addr, rawReqBytes []byte, firstSwitched time.Duration, rawRepBytes []byte, lastSwitched time.Duration) ([]byte, error) {
	...
	if len(rawReqBytes) > 0 {
		flowCount++
		dataRecords.Write(builder.buildV7Record(srcIp, SrcPort, dstIp, DstPort, firstSwitched, rawReqBytes))
	} // end if
	if len(rawRepBytes) > 0 {
		flowCount++
		dataRecords.Write(builder.buildV7Record(dstIp, DstPort, srcIp, SrcPort, lastSwitched, rawRepBytes))
	} // end if
	if flowCount <= 0 {
		return nil, nil
	} // end if
	dataHeader, errPackHeader := builder.buildV7Header(flowCount)
